...
int nRowCountB = 0,nPageCount = 0,nPage = 0;
String sId="", sRowCntB = "", sPage = "";
ResultSet rsb = null;
       rsb=(ResultSet)session.getAttribute("QUERYB");
       String user=(String)session.getAttribute("user");
       sRowCntB = (String)session.getAttribute("ROWCOUNTB");
   nRowCountB = Integer.parseInt(sRowCntB);
...
if(rsb==null)
    ...
    nPageCount = (nRowCountB+nPageLine-1)/nPageLine;
...
<html>
    <body style="background-color:white"><!--整體-->
      ...
      <h1 id="L2" class="link" style="height: 100%; position: relative; right: -65%; top: -110%; margin-left: 10%; width: 150%; color: white;" onclick="location.href='caForumManagement.jsp'">論壇管理</h1></div>
...
<%
    out.print("資料筆數:"+nRowCountB+"，頁數:第"+nPage+"頁/總頁數:"+nPageCount+"頁<hr/>");
    if(nPage > 1)  //顯示上一頁的超連結
        out.print("<a href=\"ForumManagement.jsp?page="+(nPage-1)+"\">上一頁</a>");
    if(nPage<nPageCount) //顯示下一頁
        out.print("<a href=\"ForumManagement.jsp?page="+(nPage+1)+"\">下一頁</a>");
    out.print("<a href=\"ForumManagement.jsp\">重整</a><hr/>");

rsb.absolute((nPage-1)*nPageLine+1);   //資料集指標指向此頁的第一筆
while(nLine <= nMax && !rsb.isAfterLast()){
  nLine++;
%>
...
<div id="content" style="width:85%;left:10%;height:100%;background-color:#A6D9EA;position:absolute">
    <div id='tit' style="display:initial"><%=rsb.getString("title")%></div>
</div>
<div id="edit" style="width:5%;left:95%;height:100%;background-color:white;position:absolute">
    <a class=buttonT style="background-image:url(forum/heart.png);position:absolute;bottom:0%;background-position:center;background-size:cover;right:0%;width:100%;height:33%" href="ForumManagementEdit.jsp?cid=<%=rsb.getString("id")%>"></a>
    <a class=buttonT style="background-image:url(forum/eye.png);position:absolute;bottom:33%;background-position:center;background-size:cover;right:0%;width:100%;height:33%" href="ForumSetChat.jsp?id=<%=rsb.getString("id")%>"></a>
    <a class=buttonT style="background-image:url(forum/pencil.png);position:absolute;bottom:66%;background-position:center;background-size:cover;right:0%;width:100%;height:33%"></a>
</div>
</div>
<%
          rsb.next();
