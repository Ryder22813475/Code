...
int nRowCountT = 0,nPageCount = 0,nPage = 0;
String sId="", sRowCntT = "", sPage = "";
ResultSet rst = null;
try{    
       rst=(ResultSet)session.getAttribute("QUERYT");
       String user=(String)session.getAttribute("user");
       sRowCntT = (String)session.getAttribute("ROWCOUNTT");
   nRowCountT = Integer.parseInt(sRowCntT);
}catch(Exception e){ 
    out.print("檢索結果資料存取發生錯誤:"+e.getMessage());
}
if(rst==null)
    response.sendRedirect("usehomeNT.jsp");
else{
    ...
    nPageCount = (nRowCountT+nPageLine-1)/nPageLine;
...
<html>
    <body style="background-color:white"><!--整體-->
      ...
      <h1 id="L2" class="link" style="height: 100%; position: relative; right: -65%; top: -110%; margin-left: 10%; width: 150%; color: white;" onclick="location.href='caTestManagement.jsp'">論壇管理</h1></div>
...
<%
    out.print("資料筆數:"+nRowCountT+"，頁數:第"+nPage+"頁/總頁數:"+nPageCount+"頁<hr/>");
    if(nPage > 1)  //顯示上一頁的超連結
        out.print("<a href=\"usehomeT.jsp?page="+(nPage-1)+"\">上一頁</a>");
    if(nPage<nPageCount) //顯示下一頁
        out.print("<a href=\"usehomeT.jsp?page="+(nPage+1)+"\">下一頁</a>");
    out.print("<a href=\"usehomeT.jsp\">重整</a><hr/>");

rst.absolute((nPage-1)*nPageLine+1);   //資料集指標指向此頁的第一筆
while(nLine <= nMax && !rst.isAfterLast()){
  nLine++;
%>
...
<div id="content" style="width:85%;left:10%;height:100%;background-color:#A6D9EA;position:absolute">
    <div id='tit' style="display:initial"><%=rst.getString("time")%></div>
</div>
<div id="edit" style="width:5%;left:95%;height:100%;background-color:white;position:absolute">
    <a class=buttonT style="background-image:url(forum/eye.png);position:absolute;bottom:0%;background-position:center;background-size:cover;right:0%;width:100%;height:100%" href="QQootest.jsp?answer=<%=rst.getString("Answer")%>"></a>
</div>
</div>
<%
          rst.next();
